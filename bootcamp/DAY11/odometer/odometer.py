...
def possible_readings(size: int):
    ...
    DIGITS = "123456789"
    lower_limit, higher_limit = int(DIGITS[:size]), int(DIGITS[-size:])
    return [distance for distance in range(lower_limit, higher_limit + 1) if strictly_ascending(distance) and '0' not in str(distance)]
...
def step_difference(reading1: int, reading2: int, readings: list):
    if  readings.index(reading1) < readings.index(reading2):
        return readings.index(reading2) - readings.index(reading1)
    else:
        return (len(readings) - readings.index(reading1)) + readings.index(reading2)
...
print(step_difference(789, 126, readings))